...
//K和V是u64类型的数据，8字节无符号整数;leaf_t是在trait.hpp中定义的
//alloc_t根据后面在modle_t中的使用，推测出是在leaf_allocator.hpp中定义的class LeafAllocator，这个leafAllocator应该是一个内存分配器
//remote_memory_t是在remote_memory.hh中定义的class RemoteMemory，可以在trait中跳转找到
class Rolex {
  using model_t = SubModel<K, V, leaf_t, alloc_t, Epsilon>;   
  using OptimalPLR = PLR<K, size_t>;
//model_t是子模型类而OptimalPLR则是算法模型这里主要是规定别名
private:
  remote_memory_t* RM;   //远程内存区域
...
public:
  ...
  {   //看样子是在train()函数里，完成了从训练Kv到将KV传到远端内存区域的过程
